...
func workLoop() {
  for {
    ...
    case TaskTypeMap: 
      doMap(&localProcessTask)
    case TaskTypeShuffle:
      doShuffle(&localProcessTask)
    default :
      doReduce(&localProcessTask)
    }
...
func doShuffle(task *Task) {
  ...
  for scanner.Scan() {
    ...
    fileIdx := ihash(key) % int(task.NReduce)
		output := fmt.Sprintf("shuffle-%v", fileIdx)
    out, err := os.OpenFile(output, os.O_APPEND | os.O_CREATE | os.O_WRONLY, 0666)
    if err != nil {
...
func doReduce(task *Task) {
  ...
  defer input.Close()
  output, err := os.Create(task.Output)
  if err != nil {
  ...
  os.Remove(task.Input)
	log.Printf("Reduce Result Written To %v", task.Output)
}
