...
func(rf *Raft) SyncGetLogEntries(from LogIndex, size int) []LogEntry {
	rf.logEntriesMu.RLock()
	defer rf.logEntriesMu.RUnlock()
	var entries []LogEntry
	var entriesLen int
	remain := len(rf.logEntries) - int(from)
	if remain > size { 
		entriesLen = size
	} else {
		entriesLen = remain
	...
	j := 0
	i := int(from)
	for ; i < len(rf.logEntries) && j < len(entries); {
		entries[j] = *rf.logEntries[i]
		i++
		j++ 
	}
	return entries
...
func(rf *Raft) SyncSetMatchedIndex(idx LogIndex) LogIndex {
	rf.matchedIdxMu.Lock()
	defer rf.matchedIdxMu.Unlock()
	oldMatchedIndex := rf.matchedIndex
	rf.matchedIndex = idx
	return oldMatchedIndex
}

func(rf *Raft) SyncGetMatchedIndex() LogIndex {
	rf.matchedIdxMu.RLock()
	defer rf.matchedIdxMu.RUnlock()
	return rf.matchedIndex
}

func(rf *Raft) PurgeLogEntries(lastIdx LogIndex, sync bool) {
	if sync {
		rf.logEntriesMu.Lock()
		defer rf.logEntriesMu.Unlock()
	}
	// Not include lastIdx
	rf.logEntries = rf.logEntries[:lastIdx]
}
