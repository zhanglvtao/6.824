...
func TestReElection2A(t *testing.T) {
	...
	t.Log("DEBUG: Check one leader")
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	t.Log("DEBUG: Disconnect leader")
	cfg.checkOneLeader()
	t.Logf("DEBUG: Pass disconnect leader")

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	t.Logf("DEBUG: Pass reconnect leader")

	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	cfg.disconnect((leader2 + 1) % servers)
	fmt.Printf("DEBUG: disconnect %v & %v", leader1, (leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	t.Logf("DEBUG: Pass no suffient server")

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	fmt.Printf("DEBUG: connect %v/n", (leader2 + 1) % servers)
	time.Sleep(10 * time.Second)
	cfg.checkOneLeader()
	t.Logf("DEBUG: Pass recovering from unsuffient peers")

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	t.Logf("DEBUG: Pass re-join")
...
func TestFailAgree2B(t *testing.T) {
	...
	LOG.Printf("disconnect %v", (leader + 1) % servers)
...
func TestBackup2B(t *testing.T) {
	...
	fmt.Printf("Disconnect leader1 %v follower %v", (leader1 + 0) % servers, (leader1 + 1) % servers)
	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)

	fmt.Printf("Reconnect %v %v %v", (leader1 + 2) % servers, (leader1 + 3) % servers,(leader1 + 4) % servers)
	...
	fmt.Printf("Disconnect follower %v from leader2 %v of group of 3", other, leader2)
	cfg.disconnect(other)

	fmt.Printf("Below started entry will not be committed")
	...
	fmt.Printf("Connect %v %v %v as group", (leader1 + 0) % servers, (leader1 + 1) % servers, other)
	cfg.connect((leader1 + 0) % servers)
	cfg.connect((leader1 + 1) % servers)
	cfg.connect(other)

	fmt.Printf("Below started entry should be committed")
	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}

	fmt.Printf("Connect all server")
	// now everyone
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
	cfg.one(999999, servers, true)
